'use client'

import React from 'react'
import useForm from 'react-hook-form'
import Container from '@mui/material/Container'
import Box from '@mui/material/Box'
import Typography from '@mui/material/Typography'
import TextField from '@mui/material/TextField'
import Button from '@mui/material/Button'
import BannerImg from '@/shared/components/UploadBanner'
import TipCard from '@/shared/components/TipCard'

export default function CreateEvent() {
  // const [checked, setChecked] = React.useState(true)

  // const handleChange = (event) => {
  //   setChecked(event.target.checked)
  // }

  const {
    register,
    handleSubmit,
    formState: { errors }
  } = useForm()

  const onSubmit = (data) => {
    'Dados do formulário:', data
  }

  return (
    <Container maxWidth='xl'>
      <Box sx={{ paddingTop: '4.5rem', marginTop: '2rem' }}>
        <Typography
          variant='h2'
          component='h2'
        >
          Criar novo evento
        </Typography>
        <Typography
          variant='body1'
          component='p'
          sx={{ color: '#64748B', marginTop: '1rem' }}
        >
          Compartilhe conhecimento e conecte-se com a comunidade criando seu próprio evento.
        </Typography>
      </Box>

      <Box
        component='form'
        noValidate
        onSubmit={handleSubmit(onSubmit)}
        sx={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', marginTop: '2rem' }}
      >
        <Box sx={{ flex: 1, maxWidth: '100%', minWidth: 300 }}>
          {/* Event Title */}
          <Box sx={{ display: 'flex', flexDirection: 'column', marginBottom: '2rem' }}>
            <Typography
              component='label'
              htmlFor='nomeComunidade'
              variant='subtitle1'
              fontWeight='bold'
              sx={{ marginBottom: '0.5rem' }}
            >
              Título do Evento
            </Typography>
            <TextField
              required
              id='nomeEvento'
              placeholder='Workshop React: construindo aplicações modernas'
              {...register('nomeComunidade', { required: 'Nome do Evento é Obrigatorio' })}
              error={!!errors.nomeComunidade}
              helperText={errors.nomeComunidade?.message}
              sx={{ width: '100%' }}
            />
            <Typography
              variant='caption'
              sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
            >
              Um título claro e atrativo para seu evento.
            </Typography>
          </Box>

          <Box sx={{ display: 'flex', gap: '2rem', marginBottom: '2rem', flexWrap: 'wrap' }}></Box>

          {/* Description */}
          <Box sx={{ display: 'flex', flexDirection: 'column', marginBottom: '2rem' }}>
            <Typography
              component='label'
              htmlFor='descricaoComunidade'
              variant='subtitle1'
              fontWeight='bold'
              sx={{ marginBottom: '0.5rem' }}
            >
              Descrição
            </Typography>
            <TextField
              id='descricaoComunidade'
              placeholder='Descreva o propósito da sua comunidade...'
              name='descricaoComunidade'
              type='text'
              multiline
              minRows={5}
              {...register('descricaoComunidade')}
              sx={{ width: '100%' }}
            />
            <Typography
              variant='caption'
              sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
            >
              Descreva seu evento em detalhes para atrair o público certo
            </Typography>
          </Box>

          <BannerImg />

          <Box sx={{ paddingTop: '2rem' }}>
            <Box sx={{ display: 'flex', gap: '2rem', marginBottom: '2rem', flexWrap: 'wrap', marginTop: '2rem' }}>
              <Box sx={{ flex: 1, minWidth: 200 }}>
                {/* Date */}
                <Typography
                  component='label'
                  htmlFor='data'
                  variant='subtitle1'
                  fontWeight='bold'
                  sx={{ marginBottom: '0.5rem', display: 'block' }}
                >
                  Data do Evento
                </Typography>
                <TextField
                  id='data'
                  placeholder='Selecione uma data'
                  type='date'
                  {...register('data')}
                  sx={{ width: '100%' }}
                />
                <Typography
                  variant='caption'
                  sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
                >
                  A data em que o evento ocorrerá
                </Typography>
              </Box>
              {/* Event Timepicker */}
              <Box sx={{ flex: 1, minWidth: 200 }}>
                <Typography
                  component='label'
                  variant='subtitle1'
                  fontWeight='bold'
                  sx={{ marginBottom: '0.5rem', display: 'block' }}
                >
                  Horário Inicial
                </Typography>
                <TextField
                  id='horarioinicial'
                  type='time'
                  {...register('linkedin')}
                  sx={{ width: '100%' }}
                />
                <Typography
                  variant='caption'
                  sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
                >
                  Horário que começa seu evento
                </Typography>
              </Box>
              {/* Event End-Time */}
              <Box sx={{ flex: 1, minWidth: 200 }}>
                <Typography
                  component='label'
                  variant='subtitle1'
                  fontWeight='bold'
                  sx={{ marginBottom: '0.5rem', display: 'block' }}
                >
                  Horário Final
                </Typography>
                <TextField
                  id='horarioinicial'
                  type='time'
                  placeholder='00:00'
                  {...register('linkedin')}
                  sx={{ width: '100%' }}
                />
                <Typography
                  variant='caption'
                  sx={{ mt: '0.5rem', color: 'text.secondary' }}
                >
                  Horário que termina seu evento
                </Typography>
              </Box>
            </Box>
            <Box sx={{ display: 'flex', gap: '3rem', mb: '2rem', flexWrap: 'wrap' }}>
              <Box sx={{ flex: 1, minWidth: 200 }}>
                <Typography
                  component='label'
                  variant='subtitle1'
                  fontWeight='bold'
                  sx={{ mb: '0.5rem', display: 'block' }}
                >
                  Evento Online
                </Typography>
                <Typography
                  variant='caption'
                  sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
                >
                  Marque esta opção se o evento será realizado online
                </Typography>
              </Box>
            </Box>

            <Box sx={{ display: 'flex', gap: '3rem', mb: '2rem', flexWrap: 'wrap' }}>
              <Box sx={{ flex: 1, minWidth: 200 }}>
                <Typography
                  component='label'
                  variant='subtitle1'
                  fontWeight='bold'
                  sx={{ marginBottom: '0.5rem', display: 'block' }}
                >
                  Endereço
                </Typography>
                <TextField
                  id='endereco'
                  placeholder='Av. Exemplo, 123'
                  type='url'
                  {...register('endereco')}
                  sx={{ width: '100%' }}
                />
                <Typography
                  variant='caption'
                  sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
                >
                  Endreço físico onde o evento ocorrerá
                </Typography>
              </Box>

              <Box sx={{ flex: 1, minWidth: 200 }}>
                <Typography
                  component='label'
                  htmlFor='github'
                  variant='subtitle1'
                  fontWeight='bold'
                  sx={{ marginBottom: '0.5rem', display: 'block' }}
                >
                  Cidade
                </Typography>
                <TextField
                  id='cidade'
                  placeholder='ex: Fortaleza'
                  type='url'
                  {...register('cidade')}
                  sx={{ width: '100%' }}
                />
                <Typography
                  variant='caption'
                  sx={{ marginTop: '0.5rem', color: 'text.secondary' }}
                >
                  Cidade onde o evento será realizado
                </Typography>
              </Box>
            </Box>
          </Box>

          <Button
            sx={{ backgroundColor: '#fc692d', color: '#FFFFFF', marginTop: '1rem', marginBottom: '2rem', px: '3rem' }}
            type='submit'
            variant='contained'
          >
            Criar Evento
          </Button>
        </Box>

        <TipCard />
      </Box>
    </Container>
  )
}
